    ...
    public function __construct($templatingOrDeprecatedName = null, EngineInterface $templating = null)
    {
        ...
        if (0 !== strpos(static::class, __NAMESPACE__.'\\')) {
            if (null === $templatingOrDeprecatedName) {
                @trigger_error(
                    sprintf('Passing %s as argument 1 to %s::%s() is deprecated since sonata-project/block-bundle 3.x and will throw a \TypeError as of 4.0. You must pass an instance of %s instead', \gettype($templatingOrDeprecatedName), static::class, __FUNCTION__, EngineInterface::class),
                    E_USER_DEPRECATED
                );
